...
class SpotList(APIView):
    ...
    def post(self,request,format=None):
        ...
        print(ReserveIT.get(self, request,request.data["spot_id"]))
        serializer = ParkedSerializer(data=request.data)
        if ReserveIT.get(self, request,request.data["spot_id"]):
...
# url = http://127.0.0.1:8000/spots/lat=66&log=69&r=2
# gets spot near you by giving radius and your lat , log
class SpotLatLog(APIView):
...
# url = http://127.0.0.1:8000/reserved/
class ReserveIT(APIView):
    def get(self, request, format=None):
        spots = Spots.objects.filter(occupied=True)
        serializer = SpotsSerializer(spots, many=True)
        return Response(serializer.data)

# url = http://127.0.0.1:8000/parked/
class ParkedList(APIView):
    def get(self, request, format=None):
    ...
    def post(self, request, format=None):
        ...
        serializer = ParkedSerializer(data=request.data)
        if ParkedList.get(self, request,request.data["spot_id"]):
            if serializer.is_valid():
                serializer.save()
                return redirect('/parked/')
        content = {
